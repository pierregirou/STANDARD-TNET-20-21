        ...
        if($nbLigne==0){ //Dans le cas où l'utilisateur ajoute un nouveau produit au panier
            ...
            if ($cdeMarque == '1') {
                if(!(in_array($_POST["codeMarque"],$tab)) && count($tab)>0){
                    ...
                    /* Mise à jour du stocke enlève la quantité sélectionnée et rajoute la quantité à stockencmd stockdisponible=stockdisponible-:depstock ,*/
				
					
                    $req=$bdd->prepare("UPDATE detailproduit set  stockencmd=stockencmd+:depstock  WHERE codeean13=:codeean13");
                    $req->execute(array(
        ...
        if($nbLigne>0){
            ...
            if ($cdeMarque == '1') {
                if(!(in_array($_POST["codeMarque"],$tab)) && count($tab)>0 ){
                    ...
                    if($verifQuantite>$_POST["quantite"]){ //si la quantité sélectionnée est INFERIEURE à la quantité déjà dans le panier stockdisponible=stockdisponible+:quantite ,
                    ...
                    if($verifQuantite<$_POST["quantite"]){ //si la quantité sélectionnée est SUPERIEURE à la quantité déjà dans le panier stockdisponible=stockdisponible-:quantite ,
                        $req=$bdd->prepare("UPDATE detailproduit SET  stockencmd=stockencmd+:quantite WHERE codeean13=:codeean13");
                        $req->execute(array(
