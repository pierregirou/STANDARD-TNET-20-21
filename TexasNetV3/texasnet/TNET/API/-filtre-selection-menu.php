    ...
    if(isset($_POST['login'])){
		...
		if(substr($_POST["url"], 0,1) !== '$'){
            ...
            if($modeSaisie == 2) {
                ...
                if($_POST["typeSelect"]=="produits"){
                    $req.=" GROUP BY p.refproduit,p.prix,p.tarif_promo ORDER BY p.refproduit, pg.position".$numeroMenuGalerie.",p.promo DESC";
                }
                if($_POST["typeSelect"]=="promo"){
                    $req.=" GROUP BY p.refproduit,p.prix,p.tarif_promo ORDER BY p.refproduit, pg.position".$numeroMenuGaleriePromo.",p.promo DESC";
                }
            }
		} else {
			...
			$req = "SELECT idproduit, p.refproduit, p.libelle, codeColori, codeGammeTaille, codetailledebut, codetaillefin, codeSaison,substring(mq.libelle,locate('".$_POST['langue']."',mq.libelle)+3,locate('/',substring(mq.libelle,locate('".$_POST['langue']."',mq.libelle)+3))-1) as codeMarque, substring(t.libelle,locate('".$_POST['langue']."',t.libelle)+3,locate('/',substring(t.libelle,locate('".$_POST['langue']."',t.libelle)+3))-1) as codeTheme,substring(f.libelle,locate('".$_POST['langue']."',f.libelle)+3,locate('/',substring(f.libelle,locate('".$_POST['langue']."',f.libelle)+3))-1) as codeFamille,substring(sf.libelle,locate('".$_POST['langue']."',sf.libelle)+3,locate('/',substring(sf.libelle,locate('".$_POST['langue']."',sf.libelle)+3))-1) as codeSousFamille,substring(md.libelle,locate('".$_POST['langue']."',md.libelle)+3,locate('/',substring(md.libelle,locate('".$_POST['langue']."',md.libelle)+3))-1) as codeModele,substring(l.libelle,locate('".$_POST['langue']."',l.libelle)+3,locate('/',substring(l.libelle,locate('".$_POST['langue']."',l.libelle)+3))-1) as codeLigne, nonCommandable, poids, codetarif, prix, libcolori, libMarque, commentaire1, commentaire2, commentaire3, commentaire4, commentaire5, selection, promo, tarif_promo, positionGalerie, tarif_pvc, stockdisponible, libelle2, texteLibre , pg.position1, pg.position2, pg.position3, pg.position4, pg.position5, pg.position6 FROM produit p LEFT JOIN positiongalerie pg ON p.refproduit=pg.refproduit LEFT JOIN marque mq ON p.codeMarque = mq.idMarque LEFT JOIN theme t ON p.codeTheme = t.idTheme LEFT JOIN famille f ON p.codeFamille = f.idFamille LEFT JOIN sousfamille sf ON p.codeSousFamille = sf.idSousfamille LEFT JOIN modele md ON p.codeModele = md.idModele LEFT JOIN ligne l ON p.codeLigne = l.idLigne WHERE p.codetarif='$codetarif' AND p.stockdisponible > 0 AND (";
            ...
            while($donnees=$reponse->fetch()){
                ...
                while($donnees2=$reponse2->fetch()){
                   ...
                   $picto_1 = "../Photos/Coloris/-".$donnees2["codeColori"]."-".strtoupper($donnees2["libcolori"])."-1.jpg";
                    $picto_2 = "../Photos/Coloris/-".$donnees2["codeColori"]."-".$donnees["refproduit"]."-1.jpg";
                    $picto_3 = "../Photos/Coloris/-".$donnees["codeColori"]."-".strtoupper($donnees2["libcolori"])."-".$donnees["refproduit"]."-1.jpg";

                    if(file_exists($picto_3) == 1){
                      $picto = $picto_3;
                    }elseif(file_exists($picto_2) == 1){
                      $picto = $picto_2;
                    }else{
                      $picto = $picto_1;
                    }
