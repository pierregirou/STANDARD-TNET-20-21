...
$reponse=$bdd->query("SELECT souscenTheme, visGalerie, ordreAffichage, imageMiniaturePicto FROM modules");
...
if(isset($_POST) && !empty($_POST)){
            ...
            while($donnees=$reponse->fetch()){
                ...
                $sousCentralClient=["souscentrale"];
			...
			$req = "SELECT `idproduit`, p.`refproduit`, p.`libelle`, `codeColori`, `codeGammeTaille`, `codetailledebut`, `codetaillefin`, `codeSaison`,
			...
			substring(l.libelle,locate('".$_POST['langue']."',l.libelle)+3,locate('/',substring(l.libelle,locate('".$_POST['langue']."',l.libelle)+3))-1) as codeLigne,
			 ...
			 `nonCommandable`, `poids`, `codetarif`, `prix`, `libcolori`, `libMarque`, `commentaire1`, `commentaire2`, `commentaire3`, `commentaire4`, `commentaire5`, `selection`, `promo`, `tarif_promo`, `positionGalerie`, `tarif_pvc`, `stockdisponible`, `libelle2`, `texteLibre`,`champsstat`, pg.position1, pg.position2, pg.position3, pg.position4, pg.position5, pg.position6, pgp.position1, pgp.position2, pgp.position3, pgp.position4, pgp.position5, pgp.position6
			 FROM produit p
			 ...
			 LEFT JOIN ligne l ON p.codeLigne = l.idLigne  WHERE codetarif='$codeTarif' AND nonCommandable > 0 AND stockdisponible > 0 ";
			if($souscenTheme == 1) {
                ...
                if($_POST["mode"]=="ligne"){
					...
					switch($ordreAffichage){
                        ...
                        case "4":
                            ...
                            break;
                        case "5":
                            $req .= "GROUP BY p.refproduit,prix,tarif_promo ORDER BY p.refProduit,  positionGalerie";
                            break;
                    }
	...
	while($donnees=$reponse->fetch()){
		...
		$arr2[$i]["champsstat"]=$donnees["champsstat"];
      ...
      $arr2[$i]["positionGalerie"] = $arrayposition;
		...
		$reponse3=$bdd->prepare("SELECT libcolori,codeColori,tarif_pvc,tarif_promo,promo,refproduit,codeSaison,selection FROM produit WHERE codetarif=:codetarif AND refproduit=:refproduit AND nonCommandable > 0 AND stockdisponible > 0 AND tarif_promo=:tarifPromo");
