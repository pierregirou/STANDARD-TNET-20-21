...
if(isset($_POST) && !empty($_POST)){
    ...
    while($donnees=$reponse->fetch()){
        ...
        if (file_exists("../".$picto)) {
            $libPictoArray[] = "Photos/Coloris/".$donnees['codeSaison'].$refProduit."-".$donnees['codeColori'].".jpg";
        } else {
            $libPictoArray[] = "Photos/Coloris/-".$donnees["codeColori"]."-1.jpg";
        }
    }
    ...
    while($donnees=$reponse->fetch()){
        ...
        if($donnees['codeLangue'] === "FRA"){
			$arrdetail[5][$codeLangue]=(utf8_encode(stripslashes($donnees['description'])));
    ...
    while($donnees=$reponse->fetch()){
        ...
        $fichierMin="Photos/".$codeSaison.$refProduit."-".$donnees['codeColori'].".jpg";
        ...
        for($p=1;$p<6;$p++){
            $fichierMin="Photos/".$codeSaison.$refProduit."-".$donnees['codeColori']."-".$p.".jpg";
            if(file_exists($fichierMin)){
                $tableauPhotoMini[] = $fichierMin;
            }
        }
