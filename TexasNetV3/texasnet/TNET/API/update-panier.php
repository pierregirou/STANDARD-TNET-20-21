...
if(isset($_POST) && !empty($_POST)){
    ...
    if($_POST["action"]=="plus"){
        if($stockDispo>0){
			...
			//;stockdisponible=stockdisponible-:qte ,
            $req=$bdd->prepare("UPDATE detailproduit SET  stockencmd=stockencmd+1 WHERE codeean13=:codeean13");
            $req->execute(array(
                "codeean13"=>$codeean13,
                "qte"=>$_POST["quantite"]
            ));
...
//stockdisponible=stockdisponible+:qte ,
				$req=$bdd->prepare("UPDATE detailproduit SET  stockencmd=stockencmd-:qte WHERE codeean13=:codeean13");
				$req->execute(array(
					"codeean13"=>$codeean13,
					"qte"=>($quantiteCommander - $_POST["quantite"])
				));
...
//stockdisponible=stockdisponible+:qte ,
    ...
    if($_POST["action"]=="updateMobile"){
        if($_POST["quantite"]>=1){
            ...
            /* On sélectionne une quantité inférieure à celle sélectionnée stockdisponible=stockdisponible+:quantite,*/
            if($quantiteCo>$_POST["quantite"]){
                $req=$bdd->prepare("UPDATE detailproduit SET  stockencmd=stockencmd-:quantite WHERE codeean13=:codeean13");
                $req->execute(array(
...
// stockdisponible=stockdisponible-:quantite,
            if($quantiteCo<$_POST["quantite"]){
                $res=$bdd->prepare("UPDATE detailproduit SET  stockencmd=stockencmd+:quantite WHERE codeean13=:codeean13");
                $req->execute(array(
                    "quantite"=>($quantiteCo+$_POST["quantite"]),
                    "codeean13"=>$codeean13
                ));
...
// stockdisponible=stockdisponible+:quantite ,
    ...
    if($_POST["action"]=="deleteAll"){
        ...
        while($donnees=$reponse->fetch()){
            ...
            $arrUpdate[$i]["tt"]=$tt;
            $i++;
			//stockdisponible=stockdisponible+:stockp ,
            $req=$bdd->prepare("UPDATE detailproduit SET  stockencmd=stockencmd-:stockp WHERE codeean13=:codeean13");
            $req->execute(array(
