<?php
	require_once('/home/inwitex-keolis/public_html/import/connect.php');
	session_start();
	set_time_limit(0);
		...
		if(trim($import_line) !== '' && trim($line[$pos_code_client-1]) !== '') {
			...
			if (trim($login) !== "") {
				...
				if(!$user) {
					...
					if ($civilite === "M.") { $civilite = 'M'; } else { $civilite = 'F'; }		
					$data = array("ExternalId" => $code_client, "Gender" => $civilite, "ScanType" => "H", "Action" => "s"); // data u want to post                                                                   
					$data_string = json_encode($data); 
					$login = "contact@inwitex.fr";                                                                                                                 
					$password = "Contact";   
					$userpass = $login . ":" . $password;		
					$ch = curl_init(); 
					curl_setopt($ch, CURLOPT_URL, "https://nettelows.com/gw/users/anon");    
					...
					curl_setopt($ch, CURLOPT_HTTPHEADER, array(   
						...
						'Ntws-Secret: nJo-4xlEpk-JsS-cGowHoA' 
						));       
					...
					$stmt = $db->prepare("UPDATE client SET codeSurveillance = :codeSurveillance WHERE codeClient=:codeClient");
					$stmt->execute([
						'codeClient' => $code_client,
						'codeSurveillance' => 1
						]);	
