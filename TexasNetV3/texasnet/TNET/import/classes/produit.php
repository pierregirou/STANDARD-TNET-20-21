<?php
	...
	class Produit 
	{
		public function addDetailProduit($ref,$libelle,$codecolori,$codegamme,$tailledeb,$taillefin,$saison,$marque,$theme,$famille,$sousfamille,$modele,$codeligne,$nomcmd,$description,$poids,$champsstat,$codeean13,$codetaille,$prix,$codetarif,$stockdisponible,$stockencmd,$stockaterme,$libcolori,$libmarque,$com1,$com2,$com3,$com4,$com5){
			...
			$com .= $com4;
			$com .= $com5;
			$req="insert into detailproduit values('','$ref','$codecolori','$codegamme','$tailledeb','$taillefin','$nomcmd','$codetaille','$prix','$codetarif','$codeean13','$stockdisponible','$stockencmd','$stockaterme','0')";
			echo $req;
		...
		public function getNbrProdByColTaille($sais,$ref,$col,$taille,$codetarif){
			$req="select count(*) from detailproduit where refproduit='$ref' and codeColori='$col' and codetaille='$taille' and codetarif='$codetarif'";
			echo $req."<br>";
			$red=mysql_fetch_array(mysql_query($req));
			return $red[0];
		}
		...
		public function getNbr($table,$code){
			$req="select count(*) from $table where libelle='FRA$code/'  ";
			echo $req;
			$red=mysql_fetch_array(mysql_query($req));
			return $red[0];
		}
		...
		public function addChamp($table,$libelle){
			$libelle = 'FRA'.trim($libelle).'/';
			$libelleID = ucfirst($table);
			$libelleID = "id".$libelleID."";
			$reqMax = mysql_query("SELECT MAX($libelleID) as idMax FROM $table");
			$retMax=mysql_fetch_array($reqMax);
			$idMax = $retMax['idMax'] + 1;
			$req="insert into $table (id,$libelleID,libelle) values('',$idMax,'$libelle')  ";
			echo $req."<br>";
			$red=mysql_query($req);
			return $idMax;
		}
		
		public function getChamp($table,$libelle){
			$libelle = 'FRA'.trim($libelle).'/';
			$libelleID = ucfirst($table);
			$libelleID = "id".$libelleID."";
			echo "SELECT ($libelleID) AS idMax FROM $table WHERE libelle='$libelle'";
			$reqMax = mysql_query("SELECT ($libelleID) AS idMax FROM $table WHERE libelle='$libelle'");
			$retMax=mysql_fetch_array($reqMax);
			$idMax = $retMax['idMax'];
			return $idMax;
		}
		...
		public function deleteDoublons($table){
			$req= "DELETE t1 
			FROM $table AS t1, $table AS t2
			WHERE t1.id > t2.id
			AND t1.libelle = t2.libelle";
			$exec=mysql_query($req);
