...
if ($nomFichier <> "") {
	...
	if (!$fp = fopen("./import_files/$nomFichier", "r")) {
		...
		for($i = 1; $i <= $nbrLignes; $i++) {
			...
			$data = $db->prepare("SELECT * FROM `description` WHERE codeProduit=:codeProduit AND codeSaison=:codeSaison AND codeLangue=:codeLangue");
			$data->execute([
			...
			$dataDescription = $data->fetch();
			 ...
			 if ($codeProduit[$i] !== $dataDescription["codeProduit"] && $codeSaison[$i] !== $dataDescription["codeSaison"]  && $codeLangue[$i] !== $dataDescription["codeLangue"]) {
			 	echo " ---- insert";
			 	$req = $db->prepare("INSERT INTO `description` VALUES('', :codeSaison, :codeProduit, :codeLangue, :libelleLangue, :description)");
			 	$req->execute([
			 		"codeSaison" => $codeSaison[$i],
			 		"codeProduit" => $codeProduit[$i],
			 		"codeLangue" => $codeLangue[$i],
				        "libelleLangue" => $libelleLangue[$i],
			 		"description" =>$description[$i]
			 	]);
			 	echo "<br>";
			  }else {
			  	echo " ---- update";
			  	$reqUpdate = $db->prepare("UPDATE `description` SET description=:description WHERE codeSaison=:codeSaison AND codeProduit=:codeProduit AND codeLangue=:codeLangue");
