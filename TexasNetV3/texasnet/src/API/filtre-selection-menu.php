    ...
    if(isset($_POST['login'])){
		...
		if(substr($_POST["url"], 0,1) !== '$'){
            ...
            if($_POST["typeSelect"]=="produits"){
                $req="SELECT idproduit, p.refproduit, p.libelle, codeColori, codeGammeTaille, codetailledebut, codetaillefin, codeSaison,substring(mq.libelle,locate('".$_POST['langue']."',mq.libelle)+3,locate('/',substring(mq.libelle,locate('".$_POST['langue']."',mq.libelle)+3))-1) as codeMarque, substring(t.libelle,locate('".$_POST['langue']."',t.libelle)+3,locate('/',substring(t.libelle,locate('".$_POST['langue']."',t.libelle)+3))-1) as codeTheme,substring(f.libelle,locate('".$_POST['langue']."',f.libelle)+3,locate('/',substring(f.libelle,locate('".$_POST['langue']."',f.libelle)+3))-1) as codeFamille,substring(sf.libelle,locate('".$_POST['langue']."',sf.libelle)+3,locate('/',substring(sf.libelle,locate('".$_POST['langue']."',sf.libelle)+3))-1) as codeSousFamille,substring(md.libelle,locate('".$_POST['langue']."',md.libelle)+3,locate('/',substring(md.libelle,locate('".$_POST['langue']."',md.libelle)+3))-1) as codeModele,substring(l.libelle,locate('".$_POST['langue']."',l.libelle)+3,locate('/',substring(l.libelle,locate('".$_POST['langue']."',l.libelle)+3))-1) as codeLigne, nonCommandable, poids, codetarif, prix, libcolori, libMarque, commentaire1, commentaire2, commentaire3, commentaire4, commentaire5, selection, promo, tarif_promo, positionGalerie, tarif_pvc, stockdisponible, libelle2, texteLibre , pg.position1, pg.position2, pg.position3, pg.position4, pg.position5, pg.position6 FROM produit p LEFT JOIN positiongalerie pg ON p.refproduit=pg.refproduit LEFT JOIN marque mq ON p.codeMarque = mq.idMarque LEFT JOIN theme t ON p.codeTheme = t.idTheme LEFT JOIN famille f ON p.codeFamille = f.idFamille LEFT JOIN sousfamille sf ON p.codeSousFamille = sf.idSousfamille LEFT JOIN modele md ON p.codeModele = md.idModele LEFT JOIN ligne l ON p.codeLigne = l.idLigne WHERE codetarif='$codetarif' AND p.stockdisponible > 0
				";
            }
            if($_POST["typeSelect"]=="promo"){
                $req="SELECT idproduit, p.refproduit, p.libelle, codeColori, codeGammeTaille, codetailledebut, codetaillefin, codeSaison,substring(mq.libelle,locate('".$_POST['langue']."',mq.libelle)+3,locate('/',substring(mq.libelle,locate('".$_POST['langue']."',mq.libelle)+3))-1) as codeMarque, substring(t.libelle,locate('".$_POST['langue']."',t.libelle)+3,locate('/',substring(t.libelle,locate('".$_POST['langue']."',t.libelle)+3))-1) as codeTheme,substring(f.libelle,locate('".$_POST['langue']."',f.libelle)+3,locate('/',substring(f.libelle,locate('".$_POST['langue']."',f.libelle)+3))-1) as codeFamille,substring(sf.libelle,locate('".$_POST['langue']."',sf.libelle)+3,locate('/',substring(sf.libelle,locate('".$_POST['langue']."',sf.libelle)+3))-1) as codeSousFamille,substring(md.libelle,locate('".$_POST['langue']."',md.libelle)+3,locate('/',substring(md.libelle,locate('".$_POST['langue']."',md.libelle)+3))-1) as codeModele,substring(l.libelle,locate('".$_POST['langue']."',l.libelle)+3,locate('/',substring(l.libelle,locate('".$_POST['langue']."',l.libelle)+3))-1) as codeLigne, nonCommandable, poids, codetarif, prix, libcolori, libMarque, commentaire1, commentaire2, commentaire3, commentaire4, commentaire5, selection, promo, tarif_promo, positionGalerie, tarif_pvc, stockdisponible, libelle2, texteLibre , pg.position1, pg.position2, pg.position3, pg.position4, pg.position5, pg.position6 FROM produit p LEFT JOIN positiongaleriepromo pg ON p.refproduit=pg.refproduit LEFT JOIN marque mq ON p.codeMarque = mq.idMarque LEFT JOIN theme t ON p.codeTheme = t.idTheme LEFT JOIN famille f ON p.codeFamille = f.idFamille LEFT JOIN sousfamille sf ON p.codeSousFamille = sf.idSousfamille LEFT JOIN modele md ON p.codeModele = md.idModele LEFT JOIN ligne l ON p.codeLigne = l.idLigne WHERE p.codetarif='$codetarif' AND p.promo=1
					";
            }
			...
			list($ap,$ligne,$famille,$theme,$coloris,$texteLibre,$marque,$sousFamille,$modele,$tailleD)=explode("-",$_POST["url"]);
			$langueSelect 	= 	$_POST['langue'];


			$req = "SELECT idproduit, p.refproduit, p.libelle, codeColori, codeGammeTaille, codetailledebut, codetaillefin, codeSaison,substring(mq.libelle,locate('".$_POST['langue']."',mq.libelle)+3,locate('/',substring(mq.libelle,locate('".$_POST['langue']."',mq.libelle)+3))-1) as codeMarque, substring(t.libelle,locate('".$_POST['langue']."',t.libelle)+3,locate('/',substring(t.libelle,locate('".$_POST['langue']."',t.libelle)+3))-1) as codeTheme,substring(f.libelle,locate('".$_POST['langue']."',f.libelle)+3,locate('/',substring(f.libelle,locate('".$_POST['langue']."',f.libelle)+3))-1) as codeFamille,substring(sf.libelle,locate('".$_POST['langue']."',sf.libelle)+3,locate('/',substring(sf.libelle,locate('".$_POST['langue']."',sf.libelle)+3))-1) as codeSousFamille,substring(md.libelle,locate('".$_POST['langue']."',md.libelle)+3,locate('/',substring(md.libelle,locate('".$_POST['langue']."',md.libelle)+3))-1) as codeModele,substring(l.libelle,locate('".$_POST['langue']."',l.libelle)+3,locate('/',substring(l.libelle,locate('".$_POST['langue']."',l.libelle)+3))-1) as codeLigne, nonCommandable, poids, codetarif, prix, libcolori, libMarque, commentaire1, commentaire2, commentaire3, commentaire4, commentaire5, selection, promo, tarif_promo, positionGalerie, tarif_pvc, stockdisponible, libelle2, texteLibre , pg.position1, pg.position2, pg.position3, pg.position4, pg.position5, pg.position6 FROM produit p LEFT JOIN positiongalerie pg ON p.refproduit=pg.refproduit LEFT JOIN marque mq ON p.codeMarque = mq.idMarque LEFT JOIN theme t ON p.codeTheme = t.idTheme LEFT JOIN famille f ON p.codeFamille = f.idFamille LEFT JOIN sousfamille sf ON p.codeSousFamille = sf.idSousfamille LEFT JOIN modele md ON p.codeModele = md.idModele LEFT JOIN ligne l ON p.codeLigne = l.idLigne LEFT JOIN taille ta ON p.codeGammeTaille = ta.codegamme  LEFT JOIN modele mdl ON p.codeModele = mdl.idModele WHERE p.codetarif='$codetarif' AND p.stockdisponible > 0 AND (";
      ...
      if($tailleD !== ""){
          $tailleD = str_replace(",", "','", $tailleD);
          $td =1;
          if($p === 1 || $f === 1 || $t === 1 || $c=== 1 || $tl === 1) {
              $req .=  " AND ta.libelle IN ('$tailleD') ";
          } else {
              $req .=  " ta.libelle IN ('$tailleD') ";
          }
      }
      if($marque !== ""){
          $mar = 1;
          if($p === 1 || $f === 1 || $t === 1 || $c=== 1 || $tl === 1 || $td === 1) {
              $req .=  " AND mq.libelle LIKE '%$marque%' ";
          } else {
              $req .=  " mq.libelle LIKE '%$marque%' ";
          }
      }
      if($modele !== ""){
        $mdl = 1;
        if($p === 1 || $f === 1 || $t === 1 || $c=== 1 || $tl === 1 || $td === 1 || $mar === 1) {
            $req .=  " AND mdl.libelle LIKE '%$modele%' ";
        } else {
            $req .=  " mdl.libelle LIKE '%$modele%' ";
        }
      }
      if($sousFamille !== ""){
          $ssFamille = 1;
          if($p === 1 || $f === 1 || $t === 1 || $c=== 1 || $tl === 1 || $td === 1 || $mar === 1 || $mdl === 1) {
              $req .=  " AND sf.libelle LIKE '%$sousFamille%' ";
          } else {
              $req .=  " sf.libelle LIKE '%$sousFamille%' ";
          }
              }
            ...
            while($donnees=$reponse->fetch()){
                ...
                while($donnees2=$reponse2->fetch()){
                    ...
                    $picto_1 = "Photos/Coloris/".$donnees['codeSaison'].$refProduit."-".$donnees['codeColori'].".jpg";
                    if (file_exists("../".$picto_1)) {
                        $picto= "../Photos/Coloris/".$donnees['codeSaison'].$refProduit."-".$donnees['codeColori'].".jpg";
                    }else{
                        $picto= "../Photos/Coloris/-".$donnees["codeColori"]."-1.jpg";
                    }
