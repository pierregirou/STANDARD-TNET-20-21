...
if(isset($_POST) && !empty($_POST)){
    if(isset($_POST['login'])){
        ...
        if(in_array($login,$login_array)){
            ...
            if($_POST["type"]=="marque"){ //pour récupérer les marques à filtrer
              $arrColori[0]=true;
              $reponse->closeCursor();
              $reponse=$bdd->prepare("SELECT DISTINCT substring(mqe.libelle,locate('".$_POST['langue']."',mqe.libelle)+3,locate('/',substring(mqe.libelle,locate('".$_POST['langue']."',mqe.libelle)+3))-1) as libelle FROM produit p LEFT JOIN marque mqe ON p.codeMarque = mqe.idMarque" );
              $reponse->execute();
              $i=1;
              while($donnees=$reponse->fetch()){
                  $arrMarque[$i]=$donnees["libelle"];
                  $i++;
              }
              $reponse->closeCursor();
              echo json_encode($arrMarque);
            }

          if($_POST["type"]=="sous-famille"){ //pour récupérer les sous-famille à filtrer
            $arrColori[0]=true;
            $reponse->closeCursor();
            $reponse=$bdd->prepare("SELECT DISTINCT substring(sf.libelle,locate('".$_POST['langue']."',sf.libelle)+3,locate('/',substring(sf.libelle,locate('".$_POST['langue']."',sf.libelle)+3))-1) as libelle FROM produit p LEFT JOIN sousfamille sf ON p.codeSousFamille = sf.idSousfamille");
            $reponse->execute();
            $i=1;
            while($donnees=$reponse->fetch()){
                $arrSsFamille[$i]=$donnees["libelle"];
                $i++;
            }
            $reponse->closeCursor();
            echo json_encode($arrSsFamille);
          }

          if($_POST["type"]=="modele"){ // pour récupérer les modele à filtrer
            $arrColori[0]=true;
            $reponse->closeCursor();
            $reponse=$bdd->prepare("SELECT DISTINCT substring(mdl.libelle,locate('".$_POST['langue']."',mdl.libelle)+3,locate('/',substring(mdl.libelle,locate('".$_POST['langue']."',mdl.libelle)+3))-1) as libelle FROM produit p LEFT JOIN modele mdl ON p.codeModele = mdl.idModele" );
            $reponse->execute();
            $i=1;
            while($donnees=$reponse->fetch()){
                $arrModele[$i]=$donnees["libelle"];
                $i++;
            }
            $reponse->closeCursor();
            echo json_encode($arrModele);
          }
        }
    }
}else{
